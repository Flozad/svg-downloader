// The whole trick of these clips is that a machine drew them but a person
// should appear to have driven them. Three things sell that, and nothing else
// really does:
//
//   1. Hands do not travel in straight lines, and they do not stop dead. Every
//      pointer move here rides a spring (so it decelerates into the target and
//      overshoots it by a hair) along a bowed path, not a segment.
//   2. Hands are never still. Even parked, the cursor drifts a pixel or two on
//      two incommensurable sine waves, so the drift never visibly repeats.
//   3. People type at an uneven rate, and slow down at punctuation.
//
// Everything below is frame-derived — no CSS transitions, no rAF — because
// Remotion renders frame N in isolation and anything stateful would tear.
...
export type CursorState = {
  ...
  pressed: boolean
  /** Feed to <ClickRipple/>: the click currently blooming, if any. */
...
export const useCursor = (moves: Move[], clicks: Click[] = []): CursorState => {
  ...
  for (let i = 1; i < moves.length; i++) {
    ...
    // Spring, not a bezier: the tail is what reads as a hand, and the ~2%
    // overshoot is what stops it reading as a tween.
    const p = spring({
      frame: frame - m.t * fps,
      fps,
    ...
    if (p > 0.001 && p < 0.999 && dist > 24) {
      ...
      // In-flight tremor — small, fast, and gone the moment we land.
 ...
 * beat of hesitation after punctuation, and a caret that blinks on its own
 * clock.
 */
export const useTyping = (text: string, startT: number, cps = 13) => {
  ...
  // Walk the string accumulating per-character delays. Deterministic — the
  // "noise" is a hash of the index, so frame N always agrees with frame N-1.
  let elapsed = 0
  let n = 0
  for (let i = 0; i < text.length; i++) {
    const jitter = 0.55 + ((Math.sin(i * 12.9898) * 43758.5453) % 1 + 1) % 1
    let d = jitter / cps
    if (/[.,;:?!]/.test(text[i])) d += 0.18 // punctuation is a pause
    if (text[i] === ' ') d *= 0.7
  ...
  const caret = done
    ? Math.floor((t - elapsed) * 1.8) % 2 === 0 // slow blink once idle
    : true // solid while typing — carets don't blink mid-keystroke

  return {typed: text.slice(0, n), done, caret, progress: n / text.length}
}

/**
 * A value timeline. `at(seconds, value)` pairs, eased between. The workhorse
 * for anything that isn't the pointer: scroll offsets, panel slides, opacity.
 */
...
/** True once `t0` has passed. Sugar, but it appears in every scene. */
 ...
 * that pops: menus, toasts, dialogs.
 */
